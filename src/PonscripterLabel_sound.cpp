...
int PonscripterLabel::playOGG(int format, unsigned char* buffer, long length, bool loop_flag, int channel)
{
    ...
    if (format & SOUND_OGG) {
        ...
        if(index_offset){
            ...
            long chunk_b = chunk_samples * channels * sizeof(short)/2;
            float chunk_max_volume, temp_volume;
            j=0; chunkn=0;
            while(loop_length > chunk_b*2){
                chunk_max_volume = 0.0f;
                for (i = 0; i < chunk_samples; i++){
                    temp_volume = fabs(buffer3[i]/32768.0f);
                    if(chunk_max_volume < temp_volume)
                        chunk_max_volume = temp_volume;
                }
                loop_length -= chunk_b;
                buffer3 += chunk_b;
                simul_Channel[0].timestamp[j] = chunkn*50;
                if(chunk_max_volume > EXPRESSION2THRESHOLD){
                    simul_Channel[0].infoIndex[j] = index_offset+2;
                }
                else if(chunk_max_volume > EXPRESSION1THRESHOLD){
                    simul_Channel[0].infoIndex[j] = index_offset+1;
                }
                else{
                    simul_Channel[0].infoIndex[j] = index_offset;
                }
                chunkn++;
                if(j && simul_Channel[0].infoIndex[j] == simul_Channel[0].infoIndex[j-1])
                    continue;
                j++;
            }
            simul_Channel[0].timestamp[j] = chunkn*50;
            simul_Channel[0].stampNum = j+1;
            simul_Channel[0].mode = 0;
            ...
            if(index_offset < MAX_SIMUL_NUM + 3*3)
                ...
                /*if(simul_lip_info[(index_offset - MAX_SIMUL_NUM)/3][0].affine_flag)
                    simul_Channel[0].priority = 3;
                else
                    simul_Channel[0].priority = 4;*/
            }
            simul_Channel[0].current = -1;
            simul_Channel[0].visible_with_effect = false;
        }
        ...
        if(index_offset){
            ...
            for(i=0; i<simul_Channel[0].stampNum; i++){
                simul_Channel[0].timestamp[i] += now;
            }*/
            simul_play(0);
        }
